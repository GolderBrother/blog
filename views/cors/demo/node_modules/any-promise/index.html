...
</code></pre></div><h3 id="usage-with-registration"><a href="#usage-with-registration" class="header-anchor">#</a> Usage with registration:</h3> <p>Assuming <code>bluebird</code> is the desired Promise implementation:</p> <div class="language-bash extra-class"><pre class="language-bash"><code><span class="token comment"># Install preferred promise library</span>
$ <span class="token function">npm</span> <span class="token function">install</span> bluebird
<span class="token comment"># Install any-promise to allow registration</span>
$ <span class="token function">npm</span> <span class="token function">install</span> any-promise
<span class="token comment"># Install any libraries you would like to use depending on any-promise</span>
$ <span class="token function">npm</span> <span class="token function">install</span> mz
</code></pre></div><p>Register your preference in the application entry point before any other <code>require</code> of packages that load <code>any-promise</code>:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token comment">// top of application index.js or other entry point</span>
<span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise/register/bluebird'</span><span class="token punctuation">)</span>

<span class="token comment">// -or- Equivalent to above, but allows customization of Promise library</span>
<span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise/register'</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token string">'bluebird'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>Promise<span class="token operator">:</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'bluebird'</span><span class="token punctuation">)</span><span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre></div><p>Now that the implementation is registered, you can use any package depending on <code>any-promise</code>:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">var</span> fsp <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'mz/fs'</span><span class="token punctuation">)</span> <span class="token comment">// mz/fs will use registered bluebird promises</span>
<span class="token keyword">var</span> Promise <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise'</span><span class="token punctuation">)</span>  <span class="token comment">// the registered bluebird promise </span>
</code></pre></div><p>It is safe to call <code>register</code> multiple times, but it must always be with the same implementation.</p> <p>Again, registration is <em>optional</em>. It should only be called by the application user if overriding the global <code>Promise</code> implementation is desired.</p> <h3 id="optional-application-registration"><a href="#optional-application-registration" class="header-anchor">#</a> Optional Application Registration</h3> <p>As an application author, you can <em>optionally</em> register a preferred <code>Promise</code> implementation on application startup (before any call to <code>require('any-promise')</code>:</p> <p>You must register your preference before any call to <code>require('any-promise')</code> (by you or required packages), and only one implementation can be registered. Typically, this registration would occur at the top of the application entry point.</p> <h4 id="registration-shortcuts"><a href="#registration-shortcuts" class="header-anchor">#</a> Registration shortcuts</h4> <p>If you are using a known <code>Promise</code> implementation, you can register your preference with a shortcut:</p> <div class="language-js extra-class"><pre class="language-js"><code><span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise/register/bluebird'</span><span class="token punctuation">)</span>
<span class="token comment">// -or-</span>
<span class="token keyword">import</span> <span class="token string">'any-promise/register/q'</span><span class="token punctuation">;</span>
</code></pre></div><p>Shortcut registration is the preferred registration method as it works in the browser and Node.js. It is also convenient for using with <code>import</code> and many test runners, that offer a <code>--require</code> flag:</p> <div class="language- extra-class"><pre class="language-text"><code>$ ava --require=any-promise/register/bluebird test.js
</code></pre></div><p>Current known implementations include <code>bluebird</code>, <code>q</code>, <code>when</code>, <code>rsvp</code>, <code>es6-promise</code>, <code>promise</code>, <code>native-promise-only</code>, <code>pinkie</code>, <code>vow</code> and <code>lie</code>. If you are not using a known implementation, you can use another registration method described below.</p> <h4 id="basic-registration"><a href="#basic-registration" class="header-anchor">#</a> Basic Registration</h4> <p>As an alternative to registration shortcuts, you can call the <code>register</code> function with the preferred <code>Promise</code> implementation. The benefit of this approach is that a <code>Promise</code> library can be required by name without being a known implementation.  This approach does NOT work in the browser. To use <code>any-promise</code> in the browser use either registration shortcuts or specify the <code>Promise</code> constructor using advanced registration (see below).</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise/register'</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token string">'when'</span><span class="token punctuation">)</span>
<span class="token comment">// -or- require('any-promise/register')('any other ES6 compatible library (known or otherwise)')</span>
</code></pre></div><p>This registration method will try to detect the <code>Promise</code> constructor from requiring the specified implementation.  If you would like to specify your own constructor, see advanced registration.</p> <h4 id="advanced-registration"><a href="#advanced-registration" class="header-anchor">#</a> Advanced Registration</h4> <p>To use the browser version, you should either install a polyfill or explicitly register the <code>Promise</code> constructor:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise/register'</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token string">'bluebird'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>Promise<span class="token operator">:</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'bluebird'</span><span class="token punctuation">)</span><span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre></div><p>This could also be used for registering a custom <code>Promise</code> implementation or subclass.</p> <p>Your preference will be registered globally, allowing a single registration even if multiple versions of <code>any-promise</code> are installed in the NPM dependency tree or are using multiple bundled JavaScript files in the browser. You can bypass this global registration in options:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'../register'</span><span class="token punctuation">)</span><span class="token punctuation">(</span><span class="token string">'es6-promise'</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>Promise<span class="token operator">:</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'es6-promise'</span><span class="token punctuation">)</span><span class="token punctuation">.</span>Promise<span class="token punctuation">,</span> global<span class="token operator">:</span> <span class="token boolean">false</span><span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre></div><h3 id="library-usage"><a href="#library-usage" class="header-anchor">#</a> Library Usage</h3> <p>To use any <code>Promise</code> constructor, simply require it:</p> <div class="language-javascript extra-class"><pre class="language-javascript"><code><span class="token keyword">var</span> Promise <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'any-promise'</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

<span class="token keyword">return</span> Promise
  <span class="token punctuation">.</span><span class="token function">all</span><span class="token punctuation">(</span><span class="token punctuation">[</span>xf<span class="token punctuation">,</span> f<span class="token punctuation">,</span> init<span class="token punctuation">,</span> coll<span class="token punctuation">]</span><span class="token punctuation">)</span>
  <span class="token punctuation">.</span><span class="token function">then</span><span class="token punctuation">(</span>fn<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre></div><p>Except noted below, libraries using <code>any-promise</code> should only use <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise" target="_blank" rel="noopener noreferrer">documented<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg></a> functions as there is no guarantee which implementation will be chosen by the application author.  Libraries should never call <code>register</code>, only the application user should call if desired.</p> <h4 id="advanced-library-usage"><a href="#advanced-library-usage" class="header-anchor">#</a> Advanced Library Usage</h4> <p>If your library needs to branch code based on the registered implementation, you can retrieve it using <code>var impl = require('any-promise/implementation')</code>, where <code>impl</code> will be the package name (<code>&quot;bluebird&quot;</code>, <code>&quot;when&quot;</code>, etc.) if registered, <code>&quot;global.Promise&quot;</code> if using the global version on Node.js, or <code>&quot;window.Promise&quot;</code> if using the browser version. You should always include a default case, as there is no guarantee what package may be registered.</p> <h3 id="support-for-old-node-js-versions"><a href="#support-for-old-node-js-versions" class="header-anchor">#</a> Support for old Node.js versions</h3> <p>Node.js versions prior to <code>v0.12</code> may have contained buggy versions of the global <code>Promise</code>. For this reason, the global <code>Promise</code> is not loaded automatically for these old versions.  If using <code>any-promise</code> in Node.js versions versions <code>&lt;= v0.12</code>, the user should register a desired implementation.</p> <p>If an implementation is not registered, <code>any-promise</code> will attempt to discover an installed <code>Promise</code> implementation.  If no implementation can be found, an error will be thrown on <code>require('any-promise')</code>.  While the auto-discovery usually avoids errors, it is non-deterministic. It is recommended that the user always register a preferred implementation for older Node.js versions.</p> <p>This auto-discovery is only available for Node.jS versions prior to <code>v0.12</code>. Any newer versions will always default to the global <code>Promise</code> implementation.</p> <h3 id="related"><a href="#related" class="header-anchor">#</a> Related</h3> <ul><li><a href="https://github.com/sindresorhus/any-observable" target="_blank" rel="noopener noreferrer">any-observable<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg></a> - <code>any-promise</code> for Observables.</li></ul></div> <footer class="page-edit"><!----> <div class="last-updated"><span class="prefix">Last Updated:</span> <span class="time">2020/9/7 下午8:45:37</span></div></footer> <!----> </main></div><div class="global-ui"><!----></div></div>
    <script src="/blog/assets/js/app.74817549.js" defer></script><script src="/blog/assets/js/2.f761193d.js" defer></script><script src="/blog/assets/js/53.b16b42d4.js" defer></script>
  </body>
</html>
