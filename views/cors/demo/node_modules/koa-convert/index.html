...
app<span class="token punctuation">.</span><span class="token function">use</span><span class="token punctuation">(</span>modernMiddleware<span class="token punctuation">)</span>

app<span class="token punctuation">.</span><span class="token function">use</span><span class="token punctuation">(</span><span class="token function">convert</span><span class="token punctuation">(</span>legacyMiddleware<span class="token punctuation">)</span><span class="token punctuation">)</span>

app<span class="token punctuation">.</span><span class="token function">use</span><span class="token punctuation">(</span>convert<span class="token punctuation">.</span><span class="token function">compose</span><span class="token punctuation">(</span>legacyMiddleware<span class="token punctuation">,</span> modernMiddleware<span class="token punctuation">)</span><span class="token punctuation">)</span>

<span class="token keyword">function</span> <span class="token operator">*</span> <span class="token function">legacyMiddleware</span> <span class="token punctuation">(</span><span class="token parameter">next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// before</span>
  <span class="token keyword">yield</span> next
  <span class="token comment">// after</span>
<span class="token punctuation">}</span>

<span class="token keyword">function</span> <span class="token function">modernMiddleware</span> <span class="token punctuation">(</span><span class="token parameter">ctx<span class="token punctuation">,</span> next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// before</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h2 id="distinguish-legacy-and-modern-middleware"><a href="#distinguish-legacy-and-modern-middleware" class="header-anchor">#</a> Distinguish legacy and modern middleware</h2> <p>In koa 0.x and 1.x ( without experimental flag ), <code>app.use</code> has an assertion that all ( legacy ) middleware must be generator function and it's tested with <code>fn.constructor.name == 'GeneratorFunction'</code> at <a href="https://github.com/koajs/koa/blob/7fe29d92f1e826d9ce36029e1b9263b94cba8a7c/lib/application.js#L105" target="_blank" rel="noopener noreferrer">here<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" x="0px" y="0px" viewBox="0 0 100 100" width="15" height="15" class="icon outbound"><path fill="currentColor" d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path> <polygon fill="currentColor" points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon></svg></a>.</p> <p>Therefore, we can distinguish legacy and modern middleware with <code>fn.constructor.name == 'GeneratorFunction'</code>.</p> <h2 id="migration"><a href="#migration" class="header-anchor">#</a> Migration</h2> <p><code>app.use(legacyMiddleware)</code> is everywhere in 0.x and 1.x and it would be painful to manually change all of them to <code>app.use(convert(legacyMiddleware))</code>.</p> <p>You can use following snippet to make migration easier.</p> <div class="language-js extra-class"><pre class="language-js"><code><span class="token keyword">const</span> _use <span class="token operator">=</span> app<span class="token punctuation">.</span>use
app<span class="token punctuation">.</span><span class="token function-variable function">use</span> <span class="token operator">=</span> <span class="token parameter">x</span> <span class="token operator">=&gt;</span> <span class="token function">_use</span><span class="token punctuation">.</span><span class="token function">call</span><span class="token punctuation">(</span>app<span class="token punctuation">,</span> <span class="token function">convert</span><span class="token punctuation">(</span>x<span class="token punctuation">)</span><span class="token punctuation">)</span>
</code></pre></div><p>The above snippet will override <code>app.use</code> method and implicitly convert all legacy generator middleware to modern promise middleware.</p> <p>Therefore, you can have both <code>app.use(modernMiddleware)</code> and <code>app.use(legacyMiddleware)</code> and your 0.x or 1.x should work without modification.</p> <p>Complete example:</p> <div class="language-js extra-class"><pre class="language-js"><code><span class="token keyword">const</span> Koa <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'koa'</span><span class="token punctuation">)</span> <span class="token comment">// v2.x</span>
<span class="token keyword">const</span> convert <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'koa-convert'</span><span class="token punctuation">)</span>
<span class="token keyword">const</span> app <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Koa</span><span class="token punctuation">(</span><span class="token punctuation">)</span>

<span class="token comment">// ---------- override app.use method ----------</span>

<span class="token keyword">const</span> _use <span class="token operator">=</span> app<span class="token punctuation">.</span>use
app<span class="token punctuation">.</span><span class="token function-variable function">use</span> <span class="token operator">=</span> <span class="token parameter">x</span> <span class="token operator">=&gt;</span> <span class="token function">_use</span><span class="token punctuation">.</span><span class="token function">call</span><span class="token punctuation">(</span>app<span class="token punctuation">,</span> <span class="token function">convert</span><span class="token punctuation">(</span>x<span class="token punctuation">)</span><span class="token punctuation">)</span>
...
<span class="token comment">// this will be converted to modern promise middleware implicitly</span>
app<span class="token punctuation">.</span><span class="token function">use</span><span class="token punctuation">(</span>legacyMiddleware<span class="token punctuation">)</span>
...
<span class="token keyword">function</span> <span class="token function">modernMiddleware</span> <span class="token punctuation">(</span><span class="token parameter">ctx<span class="token punctuation">,</span> next</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="api"><a href="#api" class="header-anchor">#</a> API</h2> <h4 id="convert"><a href="#convert" class="header-anchor">#</a> <code>convert()</code></h4> <p>Convert legacy generator middleware to modern promise middleware.</p> <div class="language-js extra-class"><pre class="language-js"><code>modernMiddleware <span class="token operator">=</span> <span class="token function">convert</span><span class="token punctuation">(</span>legacyMiddleware<span class="token punctuation">)</span>
</code></pre></div><h4 id="convert-compose"><a href="#convert-compose" class="header-anchor">#</a> <code>convert.compose()</code></h4> <p>Convert and compose multiple middleware (could mix legacy and modern ones) and return modern promise middleware.</p> <div class="language-js extra-class"><pre class="language-js"><code>composedModernMiddleware <span class="token operator">=</span> convert<span class="token punctuation">.</span><span class="token function">compose</span><span class="token punctuation">(</span>legacyMiddleware<span class="token punctuation">,</span> modernMiddleware<span class="token punctuation">)</span>
<span class="token comment">// or</span>
composedModernMiddleware <span class="token operator">=</span> convert<span class="token punctuation">.</span><span class="token function">compose</span><span class="token punctuation">(</span><span class="token punctuation">[</span>legacyMiddleware<span class="token punctuation">,</span> modernMiddleware<span class="token punctuation">]</span><span class="token punctuation">)</span>
</code></pre></div><h4 id="convert-back"><a href="#convert-back" class="header-anchor">#</a> <code>convert.back()</code></h4> <p>Convert modern promise middleware back to legacy generator middleware.</p> <p>This is useful to help modern promise middleware support koa 0.x or 1.x.</p> <div class="language-js extra-class"><pre class="language-js"><code>legacyMiddleware <span class="token operator">=</span> convert<span class="token punctuation">.</span><span class="token function">back</span><span class="token punctuation">(</span>modernMiddleware<span class="token punctuation">)</span>
    ...
    <script src="/blog/assets/js/app.74817549.js" defer></script><script src="/blog/assets/js/2.f761193d.js" defer></script><script src="/blog/assets/js/91.b9507740.js" defer></script>
  </body>
</html>
